...
const ContractModal = ({ tx, scope, user, onClose, onScopeUpdated }) => {
  ...
  const isClient = Boolean(
    (tx?.role && (tx.role.toLowerCase() === "buyer" || tx.role.toLowerCase() === "client")) ||
  ...
  const handleApplyNewScope = async (newScope) => {
    ...
    try {
      ...
      if (res.error) {
        ...
        if (onScopeUpdated) {
          onScopeUpdated(updatedTx || { ...tx, scope_json: newScope, title: newScope.title || tx?.title, category: newScope.category || tx?.category });
        }
      }
    } catch (err) {
  ...
  const category  = confirmedScope?.category || tx?.type || tx?.category || "Software Development";
  const totalAmt  = confirmedScope?.amount !== undefined && confirmedScope?.amount !== null ? parseFloat(confirmedScope.amount) : parseFloat(tx?.amount || 0);
    ...
    >
      <div style={{ background: T.white, borderRadius: 20, width: "100%", maxWidth: 640, maxHeight: "90vh", overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 32px 80px rgba(0,0,0,.28)", animation: "fadeUp .3s ease" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 26px" }}>
          ...
          {providerRequests.length > 0 && (
            <div style={{ background: "#fff7ed", border: "1.5px solid #fdba74", borderRadius: 10, padding: "12px 14px", fontSize: 13, color: "#c2410c", marginBottom: 16, lineHeight: 1.6 }}>
              ...
              {providerRequests.map((req, idx) => (
                <div key={idx} style={{ fontSize: 12.5, color: "#9a3412", marginTop: 4 }}>
                  • {req.note || req.metadata?.message || "Change requested"} <span style={{ fontSize: 11, opacity: 0.8 }}>({new Date(req.created_at).toLocaleDateString()})</span>
                </div>
              ))}
            </div>
          )}
          ...
          {showForm && (
            <div style={{ background: "#fff7ed", border: "1.5px solid #fdba74", borderRadius: 12, padding: "14px 16px", marginBottom: 18, animation: "fadeUp .2s ease" }}>
              <div style={{ fontWeight: 700, fontSize: 13.5, color: "#c2410c", marginBottom: 6, display: "flex", alignItems: "center", gap: 6 }}>
                <span className="msym" style={{ fontSize: 18 }}>edit_note</span>
                Request Contract Scope Changes
              </div>
              <p style={{ fontSize: 12.5, color: "#9a3412", marginBottom: 10, lineHeight: 1.5 }}>
                Describe what terms or deliverables you would like the client to modify before work begins.
              </p>
              <textarea
                rows={3}
                ...
                style={{ width: "100%", borderRadius: 8, border: "1px solid #fed7aa", padding: "8px 10px", fontSize: 13, marginBottom: 10, fontFamily: "inherit", resize: "vertical" }}
              />
        ...
        <div style={{ padding: "16px 26px", borderTop: `1px solid ${T.gray100}`, display: "flex", gap: 10, flexShrink: 0, flexWrap: "wrap" }}>
          ...
          {isProvider && canRequestChanges && (
            <Btn
              variant="outline"
              style={{ flex: 1.2, border: "1px solid #d97706", color: "#b45309" }}
