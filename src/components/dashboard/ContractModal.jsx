  ...
  const renderText =
    typeof text === "string"
      ...
      : text && typeof text === "object"
        ? text.name
          ? `${text.name}${text.description ? ": " + text.description : ""}`
          : JSON.stringify(text)
        : String(text || "");
  return (
    <div
...
const MilestoneCard = ({ m, i }) => {
  ...
  const startDate =
    m.startDate || m.start_date
      ? new Date(m.startDate || m.start_date).toLocaleDateString()
      : null;
  const dueDate =
...
const ContractModal = ({ tx, scope, user, onClose, onScopeUpdated }) => {
        ...
        .then(({ data }) => {
          if (Array.isArray(data)) {
            const reqs = data.filter(
              (e) => e.action === "contract_change_requested",
            );
  ...
  const isClient = Boolean(
    (tx?.role &&
      (tx.role.toLowerCase() === "buyer" ||
        tx.role.toLowerCase() === "client")) ||
    (user && (user.role === "buyer" || user.role === "client")) ||
    (user?.id && tx?.buyer_id && user.id === tx.buyer_id),
  );
  ...
  const handleSendChangeRequest = async () => {
    ...
    try {
      ...
      if (res.error) {
        ...
        setFeedback({
          type: "success",
          text: res.data?.message || "Change request sent to client!",
        });
        setShowForm(false);
        setRequestMsg("");
      }
  ...
  const handleApplyNewScope = async (newScope) => {
    ...
    setFeedback(null);
    try {
      const res = await transactions.confirmScope(txId, {
        scope_json: newScope,
      });
      if (res.error) {
        setFeedback({ type: "error", text: res.error });
        ...
        setFeedback({
          type: "success",
          text: "Contract scope successfully updated!",
        });
        ...
        if (onScopeUpdated) {
          onScopeUpdated(
            updatedTx || {
              ...tx,
              scope_json: newScope,
              title: newScope.title || tx?.title,
              category: newScope.category || tx?.category,
            },
          );
        }
      }
      ...
      setFeedback({
        type: "error",
        text: err.message || "Failed to update scope.",
      });
    } finally {
      setSubmitting(false);
    }
  ...
  const category =
    confirmedScope?.category ||
    tx?.type ||
    tx?.category ||
    "Software Development";
  const totalAmt =
    confirmedScope?.amount !== undefined && confirmedScope?.amount !== null
      ? parseFloat(confirmedScope.amount)
      : parseFloat(tx?.amount || 0);
  const amount = `${tx?.currency || "USD"} ${(totalAmt || 0).toLocaleString()}`;
  const buyerName =
    tx?.buyer_name ||
    (tx?.other && tx?.role === "seller" ? tx.other : "Client");
  const sellerName =
    tx?.seller_name ||
    (tx?.other && tx?.role === "buyer" ? tx.other : "Service Provider");

  const overview = confirmedScope?.overview || null;

  const deliverables =
    Array.isArray(confirmedScope?.deliverables) &&
    confirmedScope.deliverables.length
      ? confirmedScope.deliverables
      : null;
  const acceptance =
    Array.isArray(confirmedScope?.acceptance) &&
    confirmedScope.acceptance.length
      ? confirmedScope.acceptance
      : null;

  const milestones = (() => {
    if (
      Array.isArray(confirmedScope?.milestones) &&
      confirmedScope.milestones.length
    ) {
  ...
  const aiTimeline =
    confirmedScope?.timeline || tx?.ai_estimated_timeline || null;
  const agreedDuration = tx?.agreed_duration || null;
  const agreedDeadline = tx?.agreed_deadline
    ? (() => {
        const parts = String(tx.agreed_deadline)
          .split("T")[0]
          .split(" ")[0]
          .split("-")
          .map(Number);
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 26px" }}>
          ...
          {providerRequests.length > 0 && (
            ...
            >
              ...
              {providerRequests.map((req, idx) => (
                ...
                >
                  • {req.note || req.metadata?.message || "Change requested"}{" "}
          ...
          {showForm && (
            ...
            >
              ...
              >
                Describe what terms or deliverables you would like the client to
                modify before work begins.
              </p>
              <textarea
                rows={3}
                placeholder="e.g. Please extend milestone 2 target deadline by 3 days or clarify acceptance criteria for design assets..."
                value={requestMsg}
              ...
              >
                ...
                <Btn
                  ...
                  disabled={submitting || !requestMsg.trim()}
          ...
          <div
            style={{
              background: hasScope ? "#eef2ff" : "#fffbeb",
              border: `1px solid ${hasScope ? "#c7d2fe" : "#fde68a"}`,
              borderRadius: 10,
              padding: "11px 14px",
              fontSize: 13,
              color: hasScope ? "#3730a3" : "#92400e",
              lineHeight: 1.65,
              marginBottom: 20,
              display: "flex",
              alignItems: "flex-start",
          ...
          {deliverables ? (
            ...
            <>
              ...
              >
                Deliverables as described in the agreed project brief.
              </p>
            </>
          )}
          ...
          {acceptance ? (
            ...
            <>
              ...
              >
                Work accepted upon satisfactory delivery per the agreed brief.
              </p>
            </>
          )}
          ...
          {milestones ? (
            ...
            >
              Milestones distributed equally across the project value.
            </p>
          )}
          ...
          {aiTimeline && (
            ...
            >
              ...
              <div style={{ fontSize: 11.5, color: "#6366f1", marginTop: 3 }}>
                This is an AI estimate only — not the final contractual
                deadline.
              </div>
            </div>
          )}
          {agreedDuration && (
            <InfoRow label="Agreed Contract Duration" value={agreedDuration} />
          )}
          {agreedDeadline && (
            <InfoRow label="Agreed Final Deadline" value={agreedDeadline} />
          )}
          {!agreedDeadline && !agreedDuration && (
            <InfoRow
              label="Agreed Deadline"
              value="Per milestone completion schedule (no fixed date set)"
            />
          )}
          <InfoRow
            label="Review Window"
            value={`${reviewDays} business days per milestone submission`}
          />
          ...
          <div style={{ fontSize: 13, color: "#334155", lineHeight: 1.7 }}>
            ...
            <div>
              • Revisions apply strictly to the agreed scope of work above.
            </div>
          </div>
          ...
          <div style={{ fontSize: 13, color: "#334155", lineHeight: 1.7 }}>
            <div>
              • Funds of <strong>{amount}</strong> are deposited and held
              securely in Escrow regulated trust accounts.
            </div>
            <div>
              • Milestone funds are released to the Service Provider upon Client
              approval and verification.
            </div>
            <div>
              • Unresolved disputes are adjudicated via Escrow Binding
              Arbitration.
            </div>
          </div>
          ...
          >
            This Agreement is binding between the Client and Service Provider
            upon confirmation of the escrow transaction.
          </p>
        </div>
        ...
        >
          ...
          {/* Provider Role Footer (2 Buttons: Close & Request Changes) */}
          {isProvider && canRequestChanges && (
            <Btn
              variant="outline"
              style={{
                flex: 1.2,
            ...
            >
              ...
              {showForm ? "Cancel Request" : "Request Changes"}
            </Btn>
          )}
        </div>
      </div>
