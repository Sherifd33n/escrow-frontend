...
function formatApiErrorMessage(json, fallback) {
  ...
  if (json?.code === "KYC_LEVEL_REQUIRED") {
    const level = json.requiredKycLevel
      ? `Level ${json.requiredKycLevel}`
      : "";
    ...
    if (res.status === 403) {
      return {
        ...
        error: formatApiErrorMessage(
          json,
          "Action restricted. Please check your KYC verification or subscription plan."
        ),
        code: json.code,
      };
    }
...
export const transactions = {
  ...
  addMilestone: (txnId, data) =>
    post(
      `/transactions/${txnId}/milestones`,
      data
    ),

  updateMilestone: (milestoneId, data) =>
    patch(
      `/transactions/milestones/${milestoneId}/status`,
      data
    ),

  // Convenience wrapper used by dashboards:
  // PATCH /milestones/:id/status
  // { status, reason, details, deliverable_note }
  ...
  resolveDispute: (id, data) =>
    patch(
      `/transactions/${id}/dispute/resolve`,
      data
    ),
  ...
  getReviews: (id) =>
    get(`/transactions/${id}/review`),

  // Attach or update confirmed AI scope on an existing transaction (buyer only)
  confirmScope: (id, data) =>
    patch(
      `/transactions/${id}/scope`,
      data
    ),

  // Provider requests contract changes from client before starting work
  requestScopeChanges: (id, message) =>
    post(
      `/transactions/${id}/scope/request-changes`,
      { message }
    ),

  // Upload a file as submission evidence
  // returns { url, original_name, size, mime_type }
...
export const admin = {
  ...
  getDisputeAiAnalysis: (id) =>
    get(`/admin/disputes/${id}/ai-analysis`),

  triggerDisputeAiAnalysis: (id) =>
    post(
      `/admin/disputes/${id}/ai-analysis`,
      {}
    ),
...
export const payments = {
  ...
  syncPending: () =>
    post("/payments/sync-pending", {}),

  history: (page = 1) =>
    get(`/payments/history?page=${page}`),
};
...
export const bankAccounts = {
  ...
  resolve: (
    ...
    get(
      `/bank-accounts/resolve?account_number=${encodeURIComponent(
        accountNumber
...
export const users = {
  ...
  updateKYCSubmission: (id, data) =>
    patch(
      `/users/kyc/submissions/${id}`,
      data
    ),

  deleteKYCSubmission: (id) =>
    del(`/users/kyc/submissions/${id}`),
  ...
  getPortfolioQueue: () =>
    get("/users/portfolio/queue"),

  approvePortfolio: (userId) =>
    patch(
      `/users/portfolio/approve/${userId}`
    ),
  ...
  rejectPortfolio: (
    ...
    patch(
      `/users/portfolio/reject/${userId}`,
      { reason }
    ),

  send2FAOTP: () =>
    post("/users/2fa/send-otp"),
...
export const subscriptions = {
   ...
   *
   * Returns:
   * { authorization_url, reference, planId, billingCycle, amountUsd, amountNgn }.
   *
  ...
  verifyPayment: (reference) =>
    post(
      `/subscriptions/verify-payment/${encodeURIComponent(
        reference
      )}`
    ),
...
export const ai = {
  ...
  getAudits: (transactionId) =>
    get(`/ai/audits/${transactionId}`),
};
...
export function connectNotificationStream(
  ...
  const streamUrl = `${BASE}/notifications/stream?token=${encodeURIComponent(
    token
  )}`;
