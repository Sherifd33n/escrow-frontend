...
export default function VendorDashboard({ user, onLogout, onUserUpdate }) {
  ...
  const [kycIsBiz, setKycIsBiz] = useState(false);
  const [showPhoneVerify, setShowPhoneVerify] = useState(false);
  const [showPortfolioModal, setShowPortfolioModal] = useState(false);
  const [portfolioOverride, setPortfolioOverride] = useState(null);

  const portfolioVerified =
    portfolioOverride?.verified !== undefined
      ? portfolioOverride.verified
      : Boolean(user?.portfolio_verified);

  const portfolioUrl =
    portfolioOverride?.url !== undefined
      ? portfolioOverride.url
      : user?.portfolio_url || "";

  const portfolioStatus =
    portfolioOverride?.status !== undefined
      ? portfolioOverride.status
      : user?.portfolio_status || (user?.portfolio_verified ? "approved" : "none");
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    if (walletRes.data) {
      const bal =
        walletRes.data.available_balance !== undefined
  ...
  const [govtIdStatus, setGovtIdStatus] = useState("none");
  const [bizProfileStatus, setBizProfileStatus] = useState("none");
  ...
  const checkKYC = useCallback(async () => {
    try {
      ...
      if (kycRes?.data) {
        setGovtIdStatus(kycRes.data.govt_id_status || "none");
        setBizProfileStatus(kycRes.data.biz_status || "none");
      }
      if (profileRes?.data) {
        const p = profileRes.data;
        setPortfolioOverride({
          verified: Boolean(p.portfolio_verified),
          url: p.portfolio_url || "",
          status: p.portfolio_status || (p.portfolio_verified ? "approved" : "none"),
        });
      }
    } catch {
      // ignore
  ...
  useEffect(() => {
    let active = true;
    const load = async () => {
      if (active) await fetchDashboardData();
    };
    load();
    return () => {
      active = false;
  ...
  useEffect(() => {
    ...
    const offWallet = sseEmitter.on("wallet_update", () =>
      wallet.get().then((r) => {
        if (r.data) setWalletBalance(parseFloat(r.data.balance) || 0);
      }),
    );
  ...
  const totalEarned = jobs.reduce((sum, j) => {
    ...
    const approvedM = (j.milestones || []).filter(
      (m) => m.status === "approved",
    );
    return (
      sum + approvedM.reduce((mSum, m) => mSum + (parseFloat(m.amount) || 0), 0)
    );
  }, 0);
  ...
  const pendingPayout = jobs.reduce((sum, j) => {
    ...
    const isSubmitted =
      ["inspection", "audit"].includes(j.status) ||
      (j.milestones || []).some((m) => m.status === "submitted");
    if (isSubmitted) {
      const eb = parseFloat(j.escrow_balance);
      if (!isNaN(eb) && eb > 0) return sum + eb;
      const fundedM = (j.milestones || []).filter((m) =>
        ["paid", "submitted"].includes(m.status),
      );
      return (
        sum + fundedM.reduce((mSum, m) => mSum + (parseFloat(m.amount) || 0), 0)
      );
    }
    return sum;
  }, 0);

  const inEscrowValue = jobs
    ...
    .reduce((sum, j) => {
      ...
      const fundedAmt = fundedM.reduce(
        (mSum, m) => mSum + (parseFloat(m.amount) || 0),
        0,
      );
    ...
    >
        ...
        >
          <div
            style={{
              ...
              height: 42,
              borderRadius: "50%",
              background: "linear-gradient(135deg,#006c47,#005235)",
              display: "flex",
              alignItems: "center",
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 14, color: "#001637" }}>
              {user?.name || "Service Provider"}
            </div>
            <div style={{ fontSize: 11, color: "#006c47", fontWeight: 600 }}>
              Service Provider
            </div>
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          >
            <span className="msym" style={{ fontSize: 24, color: "#001637" }}>
              menu
            </span>
          </button>
          <div
            ...
            {VENDOR_TABS.map(([k, , l]) => (
              <button
                ...
                style={{
                  ...
                  borderBottom:
                    tab === k ? "2px solid #006c47" : "2px solid transparent",
                  transition: "all .15s",
                  whiteSpace: "nowrap",
                }}
              >
        ...
        >
          ...
          >
            <span className="vp-balance-label">Balance: </span>$
      ...
      >
        ...
        {tab === "overview" && (
          <div>
            <div style={{ marginBottom: 20 }}>
              ...
              <p style={{ color: "#75777f", fontSize: 14 }}>
                Manage your jobs, track earnings, and submit deliverables.
              </p>
            </div>
            ...
            >
              ...
              >
                <h2 style={{ fontSize: 16, fontWeight: 700, color: "#001637" }}>
                  Active Jobs
                </h2>
                <Btn
                  variant="ghost"
                  style={{ fontSize: 13, color: "#006c47" }}
        ...
        {tab === "jobs" && (
          <div>
            <div style={{ marginBottom: 16 }}>
              ...
              <p style={{ color: "#75777f", fontSize: 13.5, marginTop: 4 }}>
            ...
            {(() => {
              ...
              <div
                ...
                style={{
                  background: "#fff",
                  border: `1.5px solid ${detail?.id === job.id ? "#006c47" : "#e9e7eb"}`,
                  borderRadius: 14,
                  padding: "16px 18px",
                  marginBottom: 12,
                  cursor: "pointer",
                ...
                onClick={() => setDetail(detail?.id === job.id ? null : job)}
              >
                <div
                  style={{
                    display: "flex",
                  ...
                  >
                    ...
                    >
                      {[
                        [
                          "Milestones",
                          Array.isArray(job.milestones)
                            ? job.milestones.length
                            : job.milestones || 0,
                        ],
                        ["Category", job.type],
                        ["Your Role", "Service Provider"],
                        ["Status", SCFG[job.status]?.label || job.status],
                      ].map(([k, v]) => (
                        <div
                          key={k}
                          style={{
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      ...
                      {["inprogress", "revision"].includes(job.status) && (
                        ...
                        >
                          ...
                          {(job.status === "revision" ||
                            (job.milestones || []).some(
                              (m) => (m.revision_requests || []).length > 0,
                            )) && (
                            <div
                              style={{
                                background: "#fff7ed",
                              ...
                              {(job.milestones || [])
                                ...
                                .map((rr, idx) => (
                                  ...
                                  >
                                    ...
                                    {rr.details && (
                                      <div>
                                        <strong>Details:</strong> {rr.details}
                                      </div>
                                    )}
                                  </div>
                                ))}
                          ...
                          {(() => {
                            ...
                            return (
                              ...
                              >
                                ...
                                >
                                  <strong>{catLabel} Submission:</strong> Click
                                  below to enter your scope claims, evidence
                                  links, and technical artifacts for AI
                                  deliverable audit verification.
                                </div>
                      ...
                      {job.status === "funded" && (
                        <Btn
                          ...
                          onClick={async (e) => {
                            e.stopPropagation();
                            const { error } = await transactions.updateStatus(
                              job.realId,
                              "inprogress",
                            );
                            if (error) {
                      ...
                      {job.status === "inspection" && (
                        ...
                        >
                          ⏳ Client is reviewing your submission. You'll be
                          notified when done.
                        </div>
                      )}
          ...
          <div>
            ...
            >
              {[
                ...
                {
                  label: "In Escrow",
                  value: "$" + inEscrowValue.toLocaleString(),
                  color: "#1a56a0",
                },
              ].map((s) => (
                <div
                  ...
                  <tbody>
                    {jobs.map((job, i) => (
                      <tr
                        ...
                        style={{
                          borderBottom:
                            i < jobs.length - 1 ? "1px solid #f0f0f0" : "none",
                        }}
                      >
                        <td style={{ padding: "12px" }}>
                          ...
                          <div style={{ fontSize: 11, color: "#75777f" }}>
                            {job.id}
                          </div>
                        </td>
                        <td
                ...
                .map((job) => (
                  ...
                  >
                    <div>
                      ...
                      <div style={{ fontSize: 12.5, color: "#75777f" }}>
                        {job.id} · ${job.amount.toLocaleString()}
                      </div>
                    </div>
                    <Btn
                      variant="red"
                      style={{ fontSize: 13 }}
                      onClick={() => setShowDispute(job)}
                    >
                      View Details
                    </Btn>
                  </div>
        ...
        {tab === "kyc" && (
          ...
          >
            ...
            <p style={{ color: "#75777f", fontSize: 13.5, marginBottom: 20 }}>
              Verified vendors get priority placement and higher payout limits.
            </p>
            {[
              {
              ...
              },
              {
                ...
                icon: "link",
                status:
                  portfolioStatus === "approved" || portfolioVerified
                    ? "approved"
                    : portfolioStatus === "pending"
                      ...
                      : portfolioStatus === "rejected"
                        ? "rejected"
                        : "none",
                subtext: portfolioUrl || null,
                action: () => setShowPortfolioModal(true),
              },
            ].map((v) => (
              <div
                key={v.label}
      ...
      {showContract && (
        <ContractModal
          tx={showContract}
          scope={showContract?.scope_json}
          user={user}
          onClose={() => setShowContract(null)}
          onScopeUpdated={(updatedTx) => {
