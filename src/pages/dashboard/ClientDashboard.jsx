...
export default function ClientDashboard({ user, onLogout, navigate, onUserUpdate }) {
  ...
  const createTx = async () => {
    ...
    if (finalScope) {
      const existingM = Array.isArray(finalScope.milestones) ? finalScope.milestones : [];
      const expandedM = [];
    ...
    <div style={{ background:"#f5f3f6", minHeight:"100dvh", paddingBottom:80 }}>
      ...
      {showNew && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.5)", zIndex:500, display:"flex", alignItems:"center", justifyContent:"center", padding:16, backdropFilter:"blur(4px)" }} onClick={e => e.target === e.currentTarget && setShowNew(false)}>
          <div style={{ background:"#fff", borderRadius:20, width:"100%", maxWidth:560, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 24px 60px rgba(0,0,0,.25)", animation:"fadeUp .25s ease" }}>
      ...
      {showContract && <ContractModal tx={showContract} scope={showContract?.scope_json || scope} user={user} onClose={() => setShowContract(null)} onScopeUpdated={(updatedTx) => { if (updatedTx) setShowContract(updatedTx); fetchDashboardData(); }} />}
      {showScope && <ScopeModal catLabel={CATS.find(c=>c.id===nf.type)?.label||"Software"} currentAmount={nf.amount} onClose={() => setShowScope(false)} onApply={s => { setScope(s); setNf(p => ({ ...p, title: s.title || p.title, milestones: String(s.milestones_count || s.milestones?.length || 2), days: String(s.review_days || 3), amount: s.amount ? String(s.amount) : p.amount })); }} />}
