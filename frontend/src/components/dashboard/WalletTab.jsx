...
import { wallet, exchangeRate } from "../../utils/api";
...
const chargeFLWCard = async ({
  ...
  try {
    const res = await fetch(
      "https://api.flutterwave.com/v3/charges?type=card",
      {
        ...
        headers: {
          ...
          Authorization: `Bearer ${FLW_PUBLIC_KEY}`,
        },
        body: JSON.stringify({
    ...
    if (
      data.status === "success" &&
      (data.data?.status === "successful" || data.data?.status === "pending")
    ) {
      onSuccess(data.data);
    } else if (
      data.meta?.authorization?.mode === "pin" ||
      data.meta?.authorization?.mode === "avs_noauth"
    ) {
      onError("This card requires 3D-Secure. Please use another card.");
    } else {
      onError(
        data.message || "Payment declined. Check card details and try again.",
      );
    }
  } catch (e) {
    onError(
      "Network error — could not reach payment processor. Please try again.",
    );
  }
};
...
const WalletTab = ({ user, balance, onBalanceChange, activeTxs = [] }) => {
  ...
  const loadWalletData = async () => {
    const [balRes, histRes, rateRes] = await Promise.all([
      wallet.get(),
      wallet.history(),
      exchangeRate.get(),
    ]);
    ...
    console.log(histRes.data);
    console.log(histRes.data.history);
  ...
  const handleWithdraw = async () => {
    ...
    showToast(
      `$${amt.toLocaleString()} withdrawal to ${BANKS.find((b) => b.id === wdBank)?.label} initiated.`,
    );
    loadWalletData();
    setSection("overview");
    setWdAmt("");
          ...
          >
            ...
            <div>
              ...
              <div style={{ fontSize: 12.5, color: T.gray500 }}>
                Deposit from your Nigerian bank account
              </div>
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <InputField label="Select Bank" req>
              <select
                style={fs}
                value={fundBank}
                onChange={(e) => setFundBank(e.target.value)}
              >
                {BANKS.map((b) => (
        ...
        >
          ...
          {(user?.role === "seller" ||
            ...
            >
              ...
              <div>
                Your escrow earnings and wallet balance are available to
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <InputField label="Account Number" req>
              <input
                ...
                maxLength={10}
                value={wdAcct}
                onChange={(e) => setWdAcct(e.target.value.replace(/\D/g, ""))}
              />
            </InputField>
            ...
            >
              ...
              <div>
                Withdrawals processed within <strong>1–2 business days</strong>.
                A small international transfer fee may apply for non-NGN
                accounts.
              </div>
            </div>
            <Btn
              ...
              disabled={ld || !wdAmt || !wdAcct || wdAcct.length < 10}
              style={{ width: "100%", fontSize: 15 }}
            >
              {ld ? (
                <>
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <div style={{ fontSize: 12.5, color: T.gray500 }}>
                Powered by Flutterwave — AWS, GCP, Azure, GitHub and more
              </div>
            </div>
          </div>
            ...
            <>
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    >
                      <span
                        className="msym"
                        style={{ fontSize: 18, color: T.primary }}
                      >
                        credit_card
                      </span>
                      <div
                ...
                >
                  ...
                  <div>
                    Payments processed securely by <strong>Flutterwave</strong>.
                    PCI-DSS Level 1 certified. Your CVV is never stored.
                  </div>
                </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <span style={{ fontWeight: 700 }}>
                        ${parseFloat(svcAmt || 0).toLocaleString()}
                      </span>
                    </div>
                    <div
                      style={{
                    ...
                    >
                      <span>Flutterwave fee (1.4%)</span>
                      <span>
                        +${(parseFloat(svcAmt || 0) * 0.014).toFixed(2)}
                      </span>
                    </div>
                    <div
                      style={{
                    ...
                    >
                      ...
                      <span>
                        ${(parseFloat(svcAmt || 0) * 1.014).toFixed(2)}
                      </span>
                    </div>
                  </div>
                )}
                ...
                {(() => {
                  ...
                  if (cardNum.replace(/\s/g, "").length < 16)
                    missing.push(
                      `Card number needs ${16 - cardNum.replace(/\s/g, "").length} more digit${16 - cardNum.replace(/\s/g, "").length === 1 ? "" : "s"}`,
                    );
                  if (cardExp.length < 5) missing.push("Enter expiry as MM/YY");
                  if (cardCvv.length < 3)
                    missing.push("CVV needs at least 3 digits");
                  ...
                  onClick={async () => {
                    ...
                    await chargeFLWCard({
                      ...
                      onSuccess: (data) => {
                        setPayStep("success");
                        showToast(
                          `Payment of $${parseFloat(svcAmt).toLocaleString()} to ${SVCS.find((s) => s.id === svcProvider)?.label} confirmed.`,
                        );
                      },
                      onError: (msg) => {
                        setPayError(msg);
                ...
                >
                  {payStep === "processing" ? (
                    ...
                    <>
                      ...
                      Pay {SVCS.find((s) => s.id === svcProvider)?.label} via
                      Flutterwave
                    </>
                  )}
                </Btn>
